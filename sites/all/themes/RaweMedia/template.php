 ...
 */
function rawemedia_preprocess_maintenance_page(&$vars) {
  if (class_exists('Database', FALSE)) {
    // set html vars (html.tpl.php is in maintenance-page.tpl.php)
    rawemedia_preprocess_html($vars);
    // set page vars
    rawemedia_preprocess_page($vars);
  }
}
 ...
 * HTML preprocessing
 */
function rawemedia_preprocess_html(&$vars) {
  ...
  $themes = rawemedia_theme_paths($theme_key);
 ...
 * Page preprocessing
 */
function rawemedia_preprocess_page(&$vars) {
 ...
 */
function rawemedia_preprocess_region(&$vars) {
  static $grid;
 ...
 * Block preprocessing
 */
function rawemedia_preprocess_block(&$vars) {
 ...
 * Node preprocessing
 */
function rawemedia_preprocess_node(&$vars) {
  ...
  foreach($node_region_list as $region) {
    if ($list = rawemedia_block_list($region)) {
 ...
 */
function rawemedia_preprocess_comment(&$vars) {
 ...
 */
function rawemedia_preprocess_views_view(&$vars) {
 ...
 */
function rawemedia_preprocess_search_result(&$vars) {
 ...
 */
function rawemedia_region__header($vars) {
  return $vars['content'];
}
 ...
 */
function rawemedia_block_list($region) {
  $drupal_list = array();
...
function rawemedia_grid_block($vars) {
  $output = '';
  if ($vars['content']) {
    $id = $vars['id'];
...
function rawemedia_grid_info() {
  ...
  $grid['sidebar_first_width'] = (rawemedia_block_list('sidebar_first')) ? theme_get_setting('sidebar_first_width') : 0;
  $grid['sidebar_second_width'] = (rawemedia_block_list('sidebar_second')) ? theme_get_setting('sidebar_second_width') : 0;
  ...
  foreach ($regions as $region) {
    ...
    $grid['regions'][$region] = array('width' => $region_width, 'style' => $region_style, 'total' => count(rawemedia_block_list($region)), 'count' => 0);
  }
 ...
 */
function rawemedia_theme_paths($theme) {
